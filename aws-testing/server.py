...
async def run(transport: str) -> None:
    ...
    async with host.run(listen_addrs=[listen_addr]), trio.open_nursery() as nursery:
        ...
        if transport == "tcp":
            ...
            print(
                f"python client.py -d /ip4/15.188.49.159/tcp/8000/p2p/{host.get_id()} -b /ip4/15.188.49.159/tcp/8000/p2p/{host.get_id()} -p 10 -s 1"
            )  # AWS EC2 instance
        else:
            print(
                f"python client.py -t 1 -d {host.get_addrs()[0]} -p 10 -s 1"
            )  # Local
            # print(
